 ...
 */
@WebServlet("/ntcrwrite")
public class NtContestRecommentWriteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NtContestRecommentWriteServlet() {
        super();
...
//	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// vo를 가지고 서비스로 이동
		int ntcrResult = new NtContestService().insertContestRecomment(ntcrVo);

		if (ntcrResult == 0) {
			System.out.println("댓글쓰기 실패ㅠㅠ");
		} else {
			System.out.println("댓글쓰기 성공!!");
			response.sendRedirect("ntccdetail?cbNo=" + cbNo);
		}
